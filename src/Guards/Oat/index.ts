...
export class OATGuard extends BaseGuard implements OATGuardContract<any, any> {
  ...
  public async login (user: any, options?: OATLoginOptions): Promise<any> {
    ...
    this.emitter.emit(
      'auth:api:login',
      this.getLoginEventData(providerUser.user, apiToken),
    )
  ...
  public async authenticate (): Promise<any> {
    ...
    this.token = providerToken
    this.emitter.emit('auth:api:authenticate', this.getAuthenticateEventData(providerUser.user, this.token))
  ...
  public async logout () {
     ...
     */
    if (this.parsedToken) {
      await this.tokenProvider.destroy(this.parsedToken.tokenId, this.parsedToken.value, this.tokenType)
    }
