...
test.group('OAT Guard | attempt', (group) => {
  ...
  test('define custom expiry', async (assert) => {
    ...
    assert.isBelow(DateTime.fromSQL(tokens[0].expires_at).diff(DateTime.local(), 'minutes').minutes, 4)
  })
  ...
  test('define custom name for the token', async (assert) => {
    ...
    const token = await apiTokensGuard.attempt('virk@adonisjs.com', 'secret', { name: 'Android token' })
